...
4. 记录 `mechanism_trigger_decision`。
5. 若 `decision=enter_mechanism_analysis`，立即执行 `build_mechanism_argument_plan.py`；不得先写正文再补工件。
6. 对已有草稿执行 `audit_mechanism_paragraphs.py`，对新写正文至少在完成本节后执行一次。
7. 只有 `mechanism_claim_audit` 中的降级项已经写入 `evidence_gap_list.md` 或在正文中降强度后，才允许进入 Step 8。
