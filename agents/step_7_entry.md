...
- 只插入 MinerU/PDF/本地已有图片 -> `figure_backend=not_applicable`
- 从 CSV、实验数据或统计结果生成普通新图 -> `figure_backend=quick`
- 输入论文原图、截图、裁剪图、VisualSpec，或要求重绘、数字化、视觉对齐、严格 QA、SVG/PDF、manifest、checksums、语义/矢量验证 -> `figure_backend=reproduction`
- 用户显式指定 `quick/reproduction` 时覆盖自动判断；严格后端缺依赖时中止并提示安装，不得静默降级
