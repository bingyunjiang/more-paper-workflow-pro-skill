...
**4g.1b 生成 chinese_papers.json / chinese_metadata.json** 🆕

> 触发条件：检索文献表中有任意 `source=cnki` 或 `source=wanfang` 的行。无中文论文时跳过此步。

Agent 从检索文献表中提取中文论文行，写入 `chinese_papers.json`。该文件不仅供 Step 5 下载使用，也作为 Step 6 中文 Zotero 条目入库的最小元数据来源：
...
> **设计意图**：CNKI/万方中文文献多数无法通过 DOI 直接导入 Zotero。`chinese_papers.json` 必须保存足够的中文元数据，后续 Step 6 通过 CSL JSON / Zotero update 方式创建完整中文条目；`cnki.xxx` / `wanfang.xxx` 只作为 `source_id` 写入 Zotero `Extra`，不写入 DOI 字段。
