...
| # | 输出 | 格式 | 触发条件 | 说明 |
|---|------|------|----------|------|
| 9 | 饱和度曲线快照 | .json | T1-T3 文献数 ≥ 30 | 文献覆盖率估算，含置信区间；不足 30 篇时标注跳过 |
| 10 | 中文论文元数据 JSON | .json | 存在 source=cnki/wanfang 的行 | 供 Step 5 下载与 Step 6 Zotero 中文条目入库使用 |
...
| 4.3 去重合并 | 多源候选表 | DOI 归一化；无 DOI 用 title+author+year | 去重文献表 |
...
🔐 检测到 CNKI/万方需要 CARSI 机构登录。
   ...
   标题：中文数据库需要登录
   说明：检测到 CNKI / 万方需要机构账号登录后才能继续检索。你可以先完成登录，也可以先跳过中文源，仅继续英文数据库。
