...
| 🆕 7 | **中文论文 JSON** 🆕 | .json | Agent 直接写入（4g.1b 步骤） | 仅 source=cnki/wanfang 的行，供 Step 5 Chinese CDP Round 使用 |
...
**.md 正文 — 检索文献表**：
| DOI | 标题 | 作者 | 年份 | 期刊/会议 | 来源 | 评分 | Tier | 引用数 | 影响力引用 | 旗标 | 子课题 | 摘要 |
|-----|------|------|------|-----------|------|------|------|--------|------------|------|--------|------|
| 10.1016/... | Topology optimization... | Zhang Y et al. | 2024 | Int J Heat Mass Transfer | openalex | 20 | T1 | 45 | 12 | — | S1 | This paper presents a novel topology optimization approach for cold plate design... |

**.md 底部 — PRISMA-S 摘要**（可折叠区域）：
- 7 项已执行：数据库、多源、策略、日期、记录数、去重、记录管理
- 9 项未执行：标注原因"非本自动化检索范围"

**4g.1b 生成 chinese_papers.json** 🆕

> 触发条件：检索文献表中有任意 `source=cnki` 或 `source=wanfang` 的行。无中文论文时跳过此步。

Agent 从检索文献表中提取中文论文行，写入 `chinese_papers.json`：
...
| `doi` | 可选 | 真实 DOI（`10.xxxx/...`）或合成 ID（`cnki.xxx`/`wanfang.xxx`） |

> **设计意图**：Step 5 的 `parse_chinese_papers()` 原生支持 JSON 格式，字段显式无歧义，比 Markdown 表格列名匹配更可靠。此 JSON 的唯一消费者是 Step 5 `--chinese-input` 参数。
...
> - `chinese_papers.json` — 中文论文结构化数据 [若有 CNKI/Wanfang 论文] 🆕
>
...
在执行 4h 汇报之前，必须用 `ls` 或文件存在性检查确认以下 **6 个文件** 全部存在：
...
| 🆕 7 | `chinese_papers.json` | `[ -f "chinese_papers.json" ]` | 若有中文论文回到 4g.1b；若无中文论文标注「无中文论文」跳过 |
