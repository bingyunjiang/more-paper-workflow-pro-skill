...
<a id="chinese"></a>
# 📚 more paper workflow pro skill `v1.0.12-20260614`
...
- Step 7.15: `citation_audit_sample.md` 风格的引用审计结果
...
| 写后验证 | 很少做 | Step 7.15 引用审计，Step 8 只做该做的润色 |
...
Step 4: 多渠道检索+评分筛选（4.1验证→4.8完成）    → 检索文献表.md / .xlsx / .pdf / .bib
...
> 研究定题 → 文献检索 → 相关性评分 → 统一路由下载（23+ 出版社） → Zotero 入库与 PDF 附件一致性检查 → 文献证据矩阵+目标体裁/文档风格学习 → 根据文献写作（可完整写作、只写指定章节或续写已有草稿，含图表生成+引用审计） → 润色与保守修订，一站式完成学术研究文献工作。
...
选题阶段          大纲阶段          检索阶段          写作阶段          成稿阶段
   │                │                │                │                │
Step 1.4          Step 2.2          Step 4        Step 7.6/7.7/7.9       Step 7.11/7.13/7.15/8
...
(方向对不对)     (结构好不好)     (文献靠不靠谱)   (这章能不能写/这段能不能引)    (问题到底关没关/还能不能定稿)
  0-25 分          0-25 分          0-25 分       结构化工件 + 定性清单 + 证据强度     量化+叙事+动作分流
                    │                                 │                          │
               导师视角检查                    Step 7.6 章节论证计划          Step 7.11/7.13 修稿/复评
               (能不能毕业/投稿)               Step 7.7.1 可读性检查        Step 7.15 引用审计
...
| **Step 7.6 章节论证计划** | 写正文前 | 结构化约束 | 当前章节的 `core_claim` / `required_evidence` / `must_have_figure` / `rollback_if_missing` 是否清楚 | 没有证据就先不写，先回退补证据或补蓝图 |
| **Step 7.7.1 可读性检查** | 正文生成过程中 | 定性 | 段落职责、证据边界、动词强度、衔接是否足够清楚 | 只做轻量整理，不把表达偏好写成硬性质量门 |
...
| **Step 7.11/7.13 修稿与复评** | 评审后 / 修稿后 | 量化+叙事+闭环 | 问题拆解、claim 变化、修稿动作、上轮问题是否关闭、是否引入新问题 | 未关闭问题或新问题需要继续 revision-only 或回退上游 Step |
| **Step 7.15 写后引用审计** | 初稿或修稿后 | 三层审计 | `format_status / mapping_status / evidence_status` + `recommended_action`；必要时图表证据子路径 | 高风险引用不能带入定稿；必须按动作建议修复 |
| **Step 8 诊断优先润色** | 引用审计后 | 诊断+分流 | `evidence_gap / structure_drift / language_mechanical / contribution_overclaim / citation_misalignment` 哪些还能在本层修 | `ready_for_finalize / ready_with_warnings / not_ready_requires_rollback` |
...
### Step 4: 检索与评分（4.1→4.8）

> 💬 按检索方案执行多渠道文献检索，并进行引文验证、评分、分级、饱和度分析和检索报告生成。

**核心流程：**
```
4.1 引文验证 → 4.2 DOI 去重 → 4.3 筛选依据确认 → 4.4 五维评分 → 4.5 T1-T4 分级(T4剔除)
→ 4.6 引文扩展(评分闭环) → 4.7 饱和度分析 → 4.8 报告生成与完成检查
```
...
结合 `文献库.bib`、`zotero-架构.md/json` 和 PDF 附件池，生成 `文献-Zotero架构对照.md/json`、`pdf-附件池索引.json` 与 `capability_index.json/md`，明确每篇文献的推荐集合、标签、PDF 文件、导入状态、附件状态和可支持的后续 Step。`.md` 是人类审阅版，允许截断长字段；`.json` 是机器执行源，所有字段必须完整保留。PDF 附件池可包含 Step 5 下载、原有 PDF、后续补下载和手动整理目录。
...
- **章节级候选证据层（7.6.4）**：弱 RAG 只做 claim→候选证据定位，再按章节回到 note / annotation / PDF 原文确认，并把确认结果回写 `argument_plan`。
...
- **章节级论证计划（Step 7.6）**：正文生成前先锁定章节 claim、所需证据、图表要求和缺证据回退策略。
...
Step 8 可以检查明显的机械化表达、重复性问题和读者理解障碍，但不会把某种固定风格、句长节奏或“人味”写法设成硬标准。  
...
- **质量防线更新**：README 的“论文质量防线 / Quality Defense Line”已改为前中后段联动版本，覆盖 `7.6`、`7.11/7.13`、`7.15`、`8`
...
- 检索融合：Step 4 重构为 4.1→4.8 八道工序，T1→T2→T3 路由，发现曲线，arXiv 辅助检索
- 新增 3 个能力：Step 7.2 目标体裁/文档风格学习、Step 7.14 科研图表生成、Step 7.15 写后引用审计
- 术语标准化 + 错误/决策日志贯穿全流程
...
- **Figure generation (Step 7.14)**: generate reviewable figure drafts and figure inventories for paper use
- **Post-writing citation audit (Step 7.15)**: 3-layer audit (`format_status` / `mapping_status` / `evidence_status`) plus `recommended_action` for every citation
...
| 28 | 🆕 **Target Genre/Document Style Learning** | Journal/thesis/conference/existing-draft style analysis + section blueprints + writing rationale + format guard |
| 29 | 🆕 **Figure Generation** | Generate reviewable figure drafts and figure inventories for paper use |
...
| **Step 7.6 Argument Plan** | Before drafting a section | Structured contract | `core_claim` / `required_evidence` / `must_have_figure` / `rollback_if_missing` | No evidence, no drafting |
...
| **Step 7.11/7.13 Revision & Rereview** | After review / after revision | Quant+Narrative+closure | roadmap / claim delta / issue closure / new issue detection | Unresolved issues loop back |
| **Step 7.15 Citation Audit** | After draft or revision | 3-layer audit | `format_status / mapping_status / evidence_status` + `recommended_action` | High-risk citations must be fixed before finalization |
| **Step 8 Polish Diagnostics** | After audit | Diagnostic + routing | `evidence_gap / structure_drift / language_mechanical / contribution_overclaim / citation_misalignment` | finalize / warn / rollback |
...
Step 4: Multi-source search + scoring (4.1 validation→4.8 complete) → literature-table.md / .xlsx / .pdf / .bib
...
**Core workflow:**
```
4.1 Citation validation → 4.2 DOI dedup → 4.3 screening-basis confirmation → 4.4 5-dimension scoring → 4.5 T1-T4 grading(T4 removed)
→ 4.6 Citation expansion(scoring loop) → 4.7 Saturation analysis → 4.8 report generation and completion check
```
...
#### 6.4: Import Items and Attach PDFs
...
- **Figure evidence subchain added to Step 7**: introduced `figure_index.json` and `figure_evidence_report.md/json`, wiring figure/table caption binding, figure-claim checking, and figure over-interpretation risk into `argument_plan`, `revision-only`, and `7.15`
- **Quality Defense Line updated**: the README defense-line section now reflects the linked gates around `7.6`, `7.11/7.13`, `7.15`, and `8`, instead of the older 6-gate summary
...
- Search fusion: Step 4 restructured as 4.1→4.8, T1→T2→T3 routing, discovery curve, arXiv assisted search
- 3 new capabilities: Step 7.2 target genre/document style learning, Step 7.14 figure generation, Step 7.15 citation audit
