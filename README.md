...
> 研究定题 → 文献检索 → 相关性评分 → 统一路由下载（23+ 出版社） → Zotero 入库与 PDF 附件一致性检查 → 文献证据矩阵+目标体裁/文档风格学习 → 根据文献写作（可完整写作、只写指定章节或续写已有草稿，含图表生成+引用审计） → 论文润色/去AI味，一站式完成学术研究文献工作。
...
> 文献量巨大（>200 篇）时才值得做 RAG 预处理。本工具的 Step 4 检索文献表 + Step 6 Zotero 架构已完成分类，写作时优先按需精读；RAG 只在需要加速定位候选证据时作为辅助层介入。
...
选题阶段          大纲阶段          检索阶段          写作阶段          成稿阶段
   │                │                │                │                │
Step 1d          Step 2b          Step 4        Step 7.5/7.6/7.7       Step 7.9/7.11/8
...
(方向对不对)     (结构好不好)     (文献靠不靠谱)   (这章能不能写/这段能不能引)    (问题到底关没关/还能不能定稿)
  0-25 分          0-25 分          0-25 分       结构化工件 + 定性清单 + 证据强度     量化+叙事+动作分流
                    │                                 │                          │
               导师视角检查                    Step 7.5 章节论证计划          Step 7.9 修稿/复评
               (能不能毕业/投稿)               Step 7.6.1 段落自查          Step 7.11 引用审计
               workload/planB                  Step 7.7 引文评估            Step 8 诊断优先润色
...
| 节点 | 时机 | 类型 | 把控什么 | 决策 |
|------|------|:--:|------|------|
| **Step 1d 选题预审** | 方向确定后 | 量化 (0-25) | originality / importance / feasibility / literature support / method readiness | 🟢绿灯继续 / 🟡黄灯调整 / 🔴红灯换方向 |
...
| **Step 7.5 章节论证计划** | 写正文前 | 结构化约束 | 当前章节的 `core_claim` / `required_evidence` / `must_have_figure` / `rollback_if_missing` 是否清楚 | 没有证据就先不写，先回退补证据或补蓝图 |
| **Step 7.6.1 段落自查** | 每节写完后 | 定性 | 一段一工作 / 证据向外 / 动词校准 / 无虚假新颖性 / 段落流 | 写完后逐项自查，不合格即改 |
| **Step 7.7 引文评估** | 每段写完后 | 定性 | Strong / Moderate / Weak；摘要级证据是否被误用；图表 claim 是否需要额外确认 | 只引用经 Zotero 条目和 PDF 证据验证的文献；高风险 claim 触发回退或 checkpoint |
| **Step 7.9 修稿与复评** | 评审后 / 修稿后 | 量化+叙事+闭环 | 问题拆解、claim 变化、修稿动作、上轮问题是否关闭、是否引入新问题 | 限 2 轮修稿；未关闭问题或新问题需要继续 revision-only 或回退上游 Step |
| **Step 7.11 写后引用审计** | 初稿或修稿后 | 三层审计 | `format_status / mapping_status / evidence_status` + `recommended_action`；必要时图表证据子路径 | 高风险引用不能带入定稿；必须按动作建议修复 |
...
- **Step 1 / Step 2 深化重构**：Step 1 现在明确只做研究问题收敛与方法蓝图，不生成章节结构；Step 2 明确为完整大纲唯一生成层，且 `2c` 继续以已有大纲为主体，Step 1 仅作约束源
- **RAG 候选层落位**：新增 `retrieval_index_manifest.json`、`retrieval_candidates.json` 契约，明确 RAG 只负责候选定位，不能直接升级为 `VERIFIED` 或直接决定审计动作
- **图表证据子链加入 Step 7**：新增 `figure_index.json`、`figure_evidence_report.md/json` 契约，并把图注/正文绑定、图表 claim、图表误引风险接入 `argument_plan`、`revision-only` 和 `7.11`
- **质量防线更新**：README 的“论文质量防线 / Quality Defense Line”已改为前中后段联动版本，覆盖 `7.5`、`7.9`、`7.11`、`8`
...
- 新增 3 个步骤：Step 7.2 目标体裁/文档风格学习、Step 7.10 科研图表生成、Step 7.11 写后引用审计
- 术语标准化 + 错误/决策日志贯穿全流程
...
| **7.5 Argument Plan** | Before drafting a section | Structured contract | `core_claim` / `required_evidence` / `must_have_figure` / `rollback_if_missing` | No evidence, no drafting |
| **7.6.1 Paragraph Check** | After each section | Qualitative | One-job-per-¶ / Evidence-first / Verb Calibration / No False Novelty / Flow | Self-check & fix |
| **7.7 Citation Eval** | After each paragraph | Qualitative | Strong / Moderate / Weak + abstract-only misuse + figure-claim risk | Cite Zotero/PDF-verified sources; trigger rollback/checkpoint when needed |
| **7.9 Revision & Rereview** | After review / after revision | Quant+Narrative+closure | roadmap / claim delta / issue closure / new issue detection | Max 2 revision rounds; unresolved issues loop back |
| **7.11 Citation Audit** | After draft or revision | 3-layer audit | `format_status / mapping_status / evidence_status` + `recommended_action` | High-risk citations must be fixed before finalization |
| **8 Polish Diagnostics** | After audit | Diagnostic + routing | `evidence_gap / structure_drift / language_mechanical / contribution_overclaim / citation_misalignment` | finalize / warn / rollback |
...
### v1.0.12-20260614 (2026-06-14)
- **Step 1 / Step 2 refactor deepened**: Step 1 now clearly owns research-question convergence and lightweight methodology blueprinting, while Step 2 remains the only full-outline generator and `2c` still keeps the user’s existing outline as the primary artifact
...
- **Figure evidence subchain added to Step 7**: introduced `figure_index.json` and `figure_evidence_report.md/json`, wiring figure/table caption binding, figure-claim checking, and figure over-interpretation risk into `argument_plan`, `revision-only`, and `7.11`
- **Quality Defense Line updated**: the README defense-line section now reflects the linked gates around `7.5`, `7.9`, `7.11`, and `8`, instead of the older 6-gate summary
...
- 3 new steps: Step 7.2 target genre/document style learning, Step 7.10 figure generation, Step 7.11 citation audit
