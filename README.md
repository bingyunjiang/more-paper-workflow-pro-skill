...
> 研究定题 → 文献检索 → 相关性评分 → 统一路由下载（23+ 出版社） → Zotero 入库+综述矩阵+期刊风格学习 → 根据文献写作（含图表生成+引用审计） → 论文润色/去AI味，一站式完成学术研究文献工作。
...
- **大纲-集合对齐检查**：Zotero 集合与大纲章节对照，修正错放的 PDF
...
| 14 | **PDF 批量导入 Zotero** | 按 DOI 导入 + 附加 PDF，自动分类 |
| 15 | **大纲-集合对齐检查** | Zotero 集合与大纲章节对照，修正错放 PDF |
...
Step 6: Zotero 文库管理（含综述矩阵+期刊风格学习）   → zotero-架构.md + Zotero 桌面端
Step 7: 论文写作（含图表生成+引用审计+质量门评审）    → 论文初稿.md / .docx
...
PDF 导入完成后，检查 Zotero 集合结构与论文大纲章节是否对应，生成对照表。

**触发方式：** 在对话中说「检查大纲对齐」或「集合对照」。

#### 6d: 一致性调整

根据 6c 对照结果，调整 Zotero 中错放的 PDF，确保每个集合的文献与对应章节匹配。
...
> 分析目标期刊的写作风格、章节结构和论证模式，生成定制化的写作蓝图。

```bash
# 学习目标期刊风格
python3 scripts/learn_journal_style.py target-journal-info.md --output journal-style-guide.md

# 生成章节蓝皮书
python3 scripts/generate_section_blueprints.py 大纲关键词.md --style journal-style-guide.md --output section-blueprints.md

# 生成写作理由书
python3 scripts/generate_writing_rationale.py section-blueprints.md --output writing-rationale.md

# LaTeX 合规检查
python3 scripts/latex_guard.py paper-draft.md
```

**产出：** `journal-style-guide.md` + `section-blueprints.md` + `writing-rationale.md`

**参考文件：** `references/journal-style-learning-guide.md`、`references/section-blueprint-template.md`
...
│   ├── step_6_zotero.md                ← Step 6: Zotero 文库管理 + 综述矩阵 + 期刊风格
│   ├── step_7_writing.md               ← Step 7: 论文写作 + 图表生成 + 引用审计
...
> Research topic → Literature search → Relevance scoring → Unified download routing (23+ publishers) → Zotero library + review matrix + journal style learning → Writing with figures + citation audit → Paper polishing / AI-trace removal. One-stop academic research literature workflow.
...
- **Outline-collection alignment check (Step 6c/6d)**: verify Zotero collections match outline
- **Literature review matrix (Step 6e)**: 13-column structured evidence extraction with multi-level fallback
- **Journal style learning (Step 6f)**: analyze target journal writing style + section blueprints + writing rationale
...
| 14 | **Batch PDF Import to Zotero** | Import by DOI + attach PDF, auto-categorize |
...
Step 6: Zotero library management (review matrix + journal style) → zotero-architecture.md + Zotero desktop
Step 7: Paper writing (figures + citation audit + quality gate)   → paper-draft.md / .docx
...
#### 6e: Generate Literature Review Matrix 🆕

> Before writing, conduct a structured review of core literature, producing a 13-column CSV/Markdown evidence matrix.

**Trigger:** After library organization, say "generate review matrix" or "literature matrix" in the conversation.
...
> Reference: `references/literature-review-matrix-schema.md`
...
│   ├── step_6_zotero.md                ← Step 6: Zotero + review matrix + journal style
│   ├── step_7_writing.md               ← Step 7: Writing + figures + citation audit
