...
| 恢复方式 | 杀 Chrome → 删 profile → 重启 → 重新登录 | IP 认证无需重新登录 |
...
| `scripts/batch_resolve_pii.py` | 5 | DOI → PII 解析 |
| `scripts/parallel_sd_download.py` | 5 | Chrome+Edge 并行下载 |
| `scripts/auto_sd_downloader.py` | 5 | 全自动单浏览器版 |
| `scripts/setup_zotero.py` | 7 | Zotero 环境检测与配置向导 |
...
> **下一步 → Step 8：** 初稿完成后，进行精炼润色、去 AI 痕迹，提升至可投稿水平。
