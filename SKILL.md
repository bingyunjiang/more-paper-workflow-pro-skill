...
version: v1.0.15-20260619 (2026-06-19)
...
Step 4: 多渠道检索+筛选依据确认+评分+报告（4.1-4.8） → workflow_search_results.json + 检索文献表.md/.xlsx + 检索报告.md/.pdf + 文献库.bib + retrieval_index_manifest.json
  ...
  ├─ 4.2 DOI去重     多源合并去重
  ├─ 4.4 筛选依据确认 向用户展示评分维度、权重、阈值、纳入/排除规则
  ├─ 4.5 五维评分     基于用户确认的 screening_basis 做五维度 0-25 评分
  ├─ 4.6 T1-T4 分级   Tier 1-4 分级，T4 剔除或进入排除说明
  ├─ 4.7 引文扩展     单轮1-hop/T1种子/内部评分闭环
  ├─ 4.8 饱和度曲线   全量 T1-T3 覆盖率估算 + 解释 + 建议
  └─ 4.9 报告与完成   核心交付物 + 条件交付物 + retrieval_index_manifest + 转交 Step 5/6
Step 5: 统一路由下载               → paper-temp/ PDFs
Step 6: Zotero 文库管理              → zotero-架构.md + 文献-Zotero架构对照.md/json + pdf-附件池索引.json + capability_index.json/md + Zotero 条目/PDF/MinerU ZIP 附件
  ...
  ├─ 6.5 导入附件    英文按 DOI/BibTeX，中文按 source_id+CSL JSON 入库，并从 PDF 附件池关联文件
  └─ 6.6 能力索引    生成 capability_index.json/md，说明当前文献资产支持哪些后续 Step
Step 7: 论文写作（paper_type×language×target_genre） → evidence_pack.json(可选) → style_profile.md/json → section_blueprints.md/json → writing_rationale_matrix.md/json → argument_plan.md/json → retrieval_candidates.json → 论文初稿.md / 指定章节.md（完成后提示导出 .docx）
  ...
  ├─ 7.9 实时引文支撑   Zotero/证据包优先匹配→证据读取→评估→引用确认；新文献回流 Step 4/6
  ├─ 7.10 防幻觉机制    evidence_level + JSON追溯 + 中文元数据完整
  ├─ 7.11 评审与修稿路线图 评审报告 + 修稿教练 + revision_roadmap
  ├─ 7.12 revision-only 修订执行
  ├─ 7.14 复评          rereview_report + 问题闭环状态
  ├─ 7.15 图表意图与证据约束  figure_intent + evidence_basis + candidate_specs + figure_risk_note
  ├─ 7.16 写后引用审计  三层审计：format_status / mapping_status / evidence_status + recommended_action
  └─ 7.17 引用风险 checkpoint
...
> **防截断原则：** JSON、BibTeX、CSL JSON、索引 manifest 和 evidence mapping 等机器工件禁止截断；Markdown/XLSX/PDF 仅作为展示层可截断，且必须保留 `record_id`、`citekey`、`source_id`、`DOI`、`article_url`、`zotero_item_key` 等回查字段。
...
| "文献-Zotero架构对照" / "PDF 导入 Zotero" | `agents/step_6_entry.md` | 子步骤 6.3/6.5：`.json` 为完整执行源，`.md` 为审阅版；英文 DOI/BibTeX，中文 source_id/CSL JSON，多来源 PDF 附件池关联 |
...
- 当用户明确说出“选题阶段用弹窗即可”“用弹窗模式”或同义表达时，Step 1 应优先采用弹窗式收集信息。
- 若当前智能体或宿主环境不支持原生弹窗，则自动降级为“结构化文本弹窗”：
  - 一次列出当前决策包问题
  - 让用户一轮集中回答
  - 不退化成逐条拆开的零散追问
- 无论使用原生弹窗还是文本降级形式，Step 1 仍遵守 `决策包式` 收敛和原有 Step 1 -> Step 2/3 交接字段。
...
| **openpyxl** | 检索文献表 .xlsx 生成 | Step 4.9 | ✅ 必选 |
| **arxiv (>=2.1)** | arXiv API 检索 | Step 4 | ⬜ 可选 |
| **beautifulsoup4** | 万方数据 HTML 解析 | Step 4 | ⬜ 可选 |
| **PyMuPDF (fitz)** | PDF 低保真 fallback 提取 | Step 7/8 | ⬜ 可选 |
| **python-docx** | 提取/生成 .docx | Step 2.2/2.3 | ⬜ 可选 |
| **fpdf2 (>=2.5.1)** | 生成中文 PDF 报告 | Step 2.2/6.5 | ⬜ 可选 |
| **numpy** | 图表数据处理 | Step 7.16 | ⬜ 可选 |
| **matplotlib** | 科研图表生成 | Step 7.16 | ⬜ 可选 |
| **Pillow** | 图片生成/海报 | Step 7.16 | ⬜ 可选 |
| **Zotero MCP Server** | 对话读写 Zotero 文库；推荐但非 Step 7 硬依赖 | Step 6-7 | ⬜ 可选 |
...
| `scripts/arxiv_helper.py` | 4.1.9 | arXiv 新鲜度检索（L2 条件触发） |
...
| `scripts/prepare_pdf_for_llm.py` | 7/7.16/8 | 单篇或定点 PDF 提取、基础清洗、锚点分块 🆕 |
| `scripts/mineru_zip_assets.py` | 7/7.14 | 读取 Zotero MinerU ZIP（`LLM-for-Zotero-MinerU-cache-*.zip`），生成 `figure_index.json`，按需复制图片到 `figures/` |
| `scripts/citation_audit.py` | 7.16 | 写后引用审计 🆕 |
...
| `references/citation-audit-guide.md` | Step 7.16 — 引用审计方法论 |
...
| `references/nature-figure-style-guide.md` | Step 7.15 — 可选图表设计参考，不作为默认风格承诺 |
...
- Zotero/MinerU 是推荐资产层，不是硬依赖；没有 Zotero/MinerU 时走 `evidence_pack`，由用户指定 PDF、文献库、报告、数据、草稿、标准文件或图片目录
...
- 推荐 Zotero 用户安装 `llm-for-zotero` 插件；核心 PDF 可尽量用 MinerU 解析，生成的 `LLM-for-Zotero-MinerU-cache-*.zip` 通常会回挂到同一 Zotero 条目，供 Step 7 图文写作复用
- Zotero 中 MinerU 缓存的正式形态是 `LLM-for-Zotero-MinerU-cache-*.zip`；`manifest.json` 是图文索引，`full.md` 是图文阅读层，`images/` 是图片素材
- Step 7 默认先完成 Markdown 主稿，当前范围完成后再提示是否导出 DOCX；除非用户明确要求阶段 Word 审阅，不边写边导出
...
- PyMuPDF fallback 输出必须标记 `parser_confidence=low`、`must_check_pdf=true` 和布局风险；不得直接支撑图表解释、公式、表格、精确数值或强 claim
