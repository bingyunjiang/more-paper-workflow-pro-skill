...
- `deep_read_refine` 可以补充 `claim_summary`、`method_summary`、`experiment_summary`、`mechanism_hints`、`usable_for`、`not_usable_for` 和 `source_trace`，但不能提高原始 `reading_depth`。
- `mechanism_hints` 只作为 `mechanism_analysis` 的机理链候选输入；不能直接把候选变量、方程、图表或因果路径写成强 claim。
