...
当 Step 4-7 出现关键失败时，优先输出以下 4 行结构，而不是先给建议。任何关键失败不得先给大规模重跑、换源或改写方案；必须先说明观察到的症状、最可能失败层、还缺什么证据、当前最小动作。
...
- Step 4 direct-entry 默认在当前 Step 重建最小主题/RQ/范围依据并局部修复查询，不要求回跑 Step 1/2/3
- 只有用户主动要求重新定题、重做大纲或制定完整检索方案时才切换 Step；高影响范围变化可在 Step 4 内确认后继续
