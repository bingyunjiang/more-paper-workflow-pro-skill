...
def prepare_runtime(script_dir: Path, out_dir: Path, *, custom_renderer: Path | None = None) -> dict[str, Any]:
    package_dir = out_dir / "runtime" / "more_paper_figure_runtime"
...
def _render_builtin() -> int:
    ...
    from more_paper_figure_runtime.render import render_file
...
if __name__ == "__main__":
    raise SystemExit(subprocess.call([
        sys.executable,
        str(HERE / "runtime" / "more_paper_figure_runtime" / "bundle_reproduce.py"),
        "--bundle-root",
        str(HERE),
        "--qa-profile",
...
HERE = Path(__file__).resolve().parent
RUNTIME = HERE / "runtime" / "more_paper_figure_runtime"
os.environ["PYTHONDONTWRITEBYTECODE"] = "1"
