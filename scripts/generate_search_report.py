...
def build_report(
    ...
    if sat:
        ...
        cov = sat.get("coverage_estimate", 0.0)
        ci_l = sat.get("ci_lower", 0.0)
        ci_u = sat.get("ci_upper", 0.0)
        n_eval = sat.get("papers_evaluated", 0)
        n_rel = sat.get("highly_relevant_count", 0)
        n_total = sat.get("n_total_estimate", 0.0)
        lambda_val = sat.get("lambda", 0.0)
        ci_width = fq.get("ci_width", 0.0)

        lines.append(f"**生成时间**：{sat.get('timestamp', '?')}")
        lines.append("")
        lines.append(f"### {interp.get('icon', '?')} 饱和度判定：{interp.get('label_cn', '未知')}")
        lines.append("")
        ...
        if sat.get("fit_failed"):
            ...
            lines.append("|------|-----|------|")
            lines.append(f"| 文献总数 | {n_eval} | 纳入评分的 T1-T3 文献 |")
            lines.append(f"| 高相关文献 (T1) | {n_rel} | score ≥ 20，用于拟合的核心文献 |")
            lines.append(f"| 估计文献池总量 | {n_total:.0f} | 指数模型估计的高相关文献上限 |")
            lines.append(f"| 覆盖率 | {cov:.0%} | 当前 T1 文献 / 估计总量 |")
            lines.append(f"| 95% 置信区间 | {ci_l:.0%} – {ci_u:.0%} | 宽度 {ci_width:.0%} —— " + ("较窄，估计可靠" if ci_width < 0.15 else "较宽，估计不确定性高") + " |")
            lines.append(f"| 衰减系数 λ | {lambda_val:.4f} | " + ("边际发现率快速衰减 → 接近饱和" if lambda_val > 0.05 else "边际发现率稳步下降" if lambda_val > 0.01 else "边际发现率下降缓慢 → 距离饱和较远") + " |")
        lines.append("")
        ...
        lines.append("")
        lines.append("```json")
        raw = {k: v for k, v in sat.items() if k not in ("interpretation", "explanation", "recommendation", "fit_quality")}
